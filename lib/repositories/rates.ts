...
export async function updateAdminRateCard(id: string, patch: RateCardPatch, adminId?: string, options: RateCardUpdateOptions = {}): Promise<QuotationRateCardRecord | null> {
  ...
  const { data, error } = await request.select("*").maybeSingle(); if (error) throw new Error("Could not update the rate card."); if (!data) return null; const card = decorate(toRateCard(data as Record<string, unknown>)); if (patch.rate !== undefined && patch.rate !== previousRate) await saveRateCardHistory(client, { rateCardId: id, oldRate: previousRate, newRate: patch.rate, validFrom: card.validFrom, validTo: card.validTo, reason: patch.reason || "Rate updated by Admin", changedBy: adminId }); return card;
}
