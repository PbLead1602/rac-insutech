...
async function saveRateCardHistory(client: NonNullable<ReturnType<typeof getSupabaseServiceClient>>, entry: RateCardHistoryInput) {
  const auditValues = {
    rate_card_id: entry.rateCardId,
    old_rate: entry.oldRate,
    new_rate: entry.newRate,
    valid_from: entry.validFrom || null,
    valid_to: entry.validTo || null,
    reason: entry.reason,
  };
  const { error } = await client.from("quotation_rate_card_history").insert({
    ...auditValues,
    ...(entry.changedBy ? { changed_by: entry.changedBy } : {}),
  });
  if (!error) return;

  // Older production databases can have the audit table from before
  // `changed_by` was introduced. Preserve the commercial audit entry in that
  // compatible schema; the repair migration restores actor attribution.
  const missingActorColumn = entry.changedBy && error.code === "PGRST204" && /changed_by/i.test(`${error.message} ${error.details || ""}`);
  if (missingActorColumn) {
    const { error: compatibilityError } = await client.from("quotation_rate_card_history").insert(auditValues);
...
export async function createAdminRateCard(input: RateCardInput, options: RateCardCreateOptions = {}): Promise<{ card: QuotationRateCardRecord; mode: IntegrationMode }> {
  const normalizedInput = { ...input, rate: normalizeRate(input.rate) };
  ...
  const card = decorate(toRateCard(data as Record<string, unknown>)); await saveRateCardHistory(client, { rateCardId: card.id, oldRate: null, newRate: card.rate, validFrom: card.validFrom, validTo: card.validTo, reason: card.reason || "Initial rate card" }); return { card, mode };
}

export async function updateAdminRateCard(id: string, patch: RateCardPatch, adminId?: string, options: RateCardUpdateOptions = {}): Promise<QuotationRateCardRecord | null> {
  const mode = integrationMode(serverEnv.supabaseServiceConfigured); const now = new Date().toISOString(); const normalizedRate = patch.rate === undefined ? undefined : normalizeRate(patch.rate);
  if (mode === "mock") { const index = developmentStore().cards.findIndex((item) => item.id === id); if (index < 0) return null; const before = developmentStore().cards[index]; const card = { ...before, ...patch, ...(normalizedRate === undefined ? {} : { rate: normalizedRate }), active: patch.active === undefined ? before.active : patch.active, publishedAt: patch.active === true && !before.active ? now : before.publishedAt, archivedAt: patch.archivedAt === undefined ? before.archivedAt : patch.archivedAt || undefined }; developmentStore().cards[index] = card; if (normalizedRate !== undefined && normalizedRate !== before.rate) developmentStore().history.unshift({ id: randomUUID(), rateCardId: id, oldRate: before.rate, newRate: normalizedRate, validFrom: card.validFrom, validTo: card.validTo, reason: patch.reason || "Rate updated by Admin", changedAt: now }); return card; }
  if (mode === "unconfigured") throw new Error("Rate-card storage is not configured."); const client = getSupabaseServiceClient(); if (!client) throw new Error("Supabase service client is unavailable.");
  let previousRate = options.expectedPreviousRate;
  if (previousRate === undefined) {
    const { data: previous, error: previousError } = await client.from("quotation_rate_cards").select("rate").eq("id", id).maybeSingle();
    if (previousError) throw new Error("Could not load the previous rate.");
    if (!previous) return null;
    previousRate = Number(previous.rate);
  }
  const update = { ...(normalizedRate !== undefined ? { rate: normalizedRate } : {}), ...(patch.gstRate !== undefined ? { gst_rate: patch.gstRate } : {}), ...(patch.active !== undefined ? { active: patch.active } : {}), ...(patch.validFrom !== undefined ? { valid_from: patch.validFrom || null } : {}), ...(patch.validTo !== undefined ? { valid_to: patch.validTo || null } : {}), ...(patch.reason !== undefined ? { reason: patch.reason || null } : {}), ...(patch.packingLabel !== undefined ? { packing_label: patch.packingLabel || null } : {}), ...(patch.moq !== undefined ? { moq: patch.moq || null } : {}), ...(patch.rollAreaM2 !== undefined ? { roll_area_m2: patch.rollAreaM2 || null } : {}), ...(patch.packRunningMetres !== undefined ? { pack_running_metres: patch.packRunningMetres || null } : {}), ...(patch.archivedAt !== undefined ? { archived_at: patch.archivedAt || null } : {}), ...(patch.active === true ? { published_at: now } : {}) };
  let request = client.from("quotation_rate_cards").update(update).eq("id", id);
  if (options.expectedPreviousRate !== undefined) request = request.eq("rate", options.expectedPreviousRate);
  const { data, error } = await request.select("*").maybeSingle(); if (error) throw new Error("Could not update the rate card."); if (!data) return null; const card = decorate(toRateCard(data as Record<string, unknown>)); if (normalizedRate !== undefined && normalizedRate !== previousRate) await saveRateCardHistory(client, { rateCardId: id, oldRate: previousRate, newRate: normalizedRate, validFrom: card.validFrom, validTo: card.validTo, reason: patch.reason || "Rate updated by Admin", changedBy: adminId }); return card;
}
