...
import { calculateQuotationTotals, parseStoredTransport, resolveTransport, type TransportMode } from "@/lib/quotations/transport";
...
export async function createAdminQuotation(input: CreateAdminQuotationInput): Promise<SaveQuotationResult> {
  ...
  const totals = calculateQuotationTotals(subtotal, input.gstRate, transport.charge);
...
export async function createAdminQuotationRevision(id: string, input: CreateQuotationRevisionInput): Promise<QuotationRecord | null> {
  ...
  if (mode === "mock") {
    ...
    const identity = revisionNumberAndQuoteNumber(source, family); const items = revisedLineItems(input.items); const subtotal = Number(items.reduce((total, item) => total + item.amount, 0).toFixed(2)); const transport = input.transportMode ? resolveTransport(input.transportMode, input.transportCharge) : parseStoredTransport(source.transport); const totals = calculateQuotationTotals(subtotal, input.gstRate, transport.charge);
    const quotation: QuotationRecord = { ...source, id: randomUUID(), quoteNumber: identity.quoteNumber, accessToken: randomUUID().replaceAll("-", ""), customer: input.customer, items, subtotal: totals.subtotal, gstRate: input.gstRate, gstAmount: totals.gstAmount, total: totals.total, transport: transport.label, validUntil: input.validUntil || validityDate(createdAt, source.validityDays), internalNotes: input.internalNotes || source.internalNotes, source: "admin_created", revisionNumber: identity.revisionNumber, parentQuotationId: rootId, status: "revised", createdAt };
