...
export const indiaCityPinCodeOptions: readonly IndiaCityPinCodeOption[] = [
...
export function searchIndiaCities(query: string) {
  const needle = normalized(query);
  if (!needle) return [];
  return indiaCityPinCodeOptions.filter((option) => normalized(option.city).startsWith(needle) || option.aliases?.some((alias) => normalized(alias).startsWith(needle)));
}
