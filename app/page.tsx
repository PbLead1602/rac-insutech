...
function QuoteModal({ initialProduct, onClose }: { initialProduct: string; onClose: () => void }) {
  ...
  return <motion.div className="modal-backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={onClose}><motion.div className="quote-modal" initial={{ opacity: 0, y: 18, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.98 }} onMouseDown={(event) => event.stopPropagation()}><button className="modal-close" onClick={onClose} aria-label="Close enquiry form"><X size={20} /></button><div className="modal-kicker"><span /> START AN ENQUIRY</div><h2>Tell us about your requirement.</h2><p>Your enquiry is saved first. Commercial quotation generation is available after sign-in and RAC Admin approval.</p><form onSubmit={submitQuote}><div className="form-row"><label>Full name<input name="name" required placeholder="Your name" /></label><label>Company<input name="company" required placeholder="Company name" /></label></div><div className="form-row"><label>Mobile number<input name="mobile" required type="tel" placeholder="+91" /></label><label>Email<input name="email" required type="email" placeholder="you@company.com" /></label></div><div className="form-row"><label>City<input name="city" placeholder="Your city" /></label><label>State<input name="state" placeholder="Your state" /></label></div><div className="form-row"><label>PIN code<input name="pinCode" inputMode="numeric" placeholder="Postal code" /></label><label>Customer type<select name="customerType" defaultValue="end_user"><option value="end_user">End user</option><option value="contractor">Contractor</option><option value="consultant">Consultant</option><option value="dealer">Dealer</option><option value="other">Other</option></select></label></div><div className="form-row"><label>Project name<input name="projectName" placeholder="Optional project name" /></label><label>Project location<input name="projectLocation" placeholder="Site location" /></label></div><div className="form-row"><label>Product or material<input name="product" value={product} onChange={(event) => setProduct(event.target.value)} placeholder="e.g. XLPE sheet insulation" /></label><label>Application<input name="application" placeholder="e.g. HVAC ducting" /></label></div><div className="form-row"><label>Brand preference<input name="brand" placeholder="Optional" /></label><label>Delivery preference<input name="deliveryPreference" placeholder="Optional timeline / location" /></label></div><div className="form-row"><label>Quantity<input name="quantity" placeholder="e.g. 250 m²" /></label><label>Thickness / size<input name="thickness" placeholder="e.g. 25 mm" /></label></div><label>Supporting file <small>Optional: PDF, BOQ, XLSX, DWG, PNG or JPG — up to 10 MB.</small><input className="file-input" name="attachment" type="file" accept=".pdf,.xlsx,.xls,.dwg,image/png,image/jpeg" /></label><label>Tell us about your requirement<textarea name="message" placeholder="Operating temperature, timeline, delivery needs or other helpful details." rows={3} /></label><TurnstileWidget onVerify={setTurnstileToken} />{error && <p className="form-error" role="alert">{error}</p>}<button className="button button-gradient full-width" type="submit" disabled={busy}>{busy ? "Saving enquiry..." : "Continue securely"}<ArrowRight size={18} /></button></form></motion.div></motion.div>;
}
