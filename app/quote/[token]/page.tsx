...
import { getQuotationByAccessToken, updateAdminQuotation } from "@/lib/repositories/quotations";
...
export default async function SecureQuotationPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  let quotation = await getQuotationByAccessToken(token);
  if (!quotation) notFound();
  if (["generated", "sent", "revised"].includes(quotation.status)) quotation = (await updateAdminQuotation(quotation.id, { status: "viewed" })) || quotation;
  return <main className="quotation-page"><CatalogueHeader /><section className="secure-quote catalogue-shell">
    <div className="secure-quote-top"><div><p className="catalogue-kicker"><span /> SECURE QUOTATION</p><h1>{quotation.quoteNumber}</h1><p>Prepared for {quotation.customer.company}. Valid for {quotation.validityDays} days from issue.</p></div><Link className="quotation-primary" href={`/account/quotations/${quotation.id}`}><FileText size={17} /> Open in customer portal</Link></div>
    <div className="secure-quote-customer"><div><span>Customer</span><strong>{quotation.customer.fullName}</strong><p>{quotation.customer.company}</p></div><div><span>Project</span><strong>{quotation.customer.projectName || "To be confirmed"}</strong><p>{quotation.customer.projectLocation || quotation.customer.city || "Location to be confirmed"}</p></div><div><span>Quote status</span><strong>{quotationStatusLabel(quotation.status)}</strong><p>Transport: {quotation.transport}</p></div></div>
    <div className="secure-quote-lines"><div className="secure-quote-heading"><span>Sr No</span><span>Product</span><span>Supply quantity</span><span>Rate</span><span>Amount</span></div>{quotation.items.map((item, index) => <div className={`secure-quote-line${item.customBuiltUp ? " custom-built-up" : ""}`} key={`${item.variantId}-${index}`}><span className="secure-quote-serial">{index + 1}</span><span><strong>{item.productName}</strong><small>{item.configuration}</small><BuiltUpNbrQuoteDetails item={item} /></span>{item.customBuiltUp ? <BuiltUpNbrCommercialColumns item={item} /> : <><span>{item.technicalQuantity}</span><span>{currency.format(item.rate)}<small>{item.rateUnit}</small></span><strong>{currency.format(item.amount)}</strong></>}</div>)}</div>
    <div className="secure-quote-bottom"><div className="secure-quote-note"><ShieldCheck size={19} /><p><strong>Review required before order acceptance.</strong> Sign in to your approved RAC account to download the protected PDF or request a revision.</p></div><div className="secure-quote-total"><span>Subtotal <b>{currency.format(quotation.subtotal)}</b></span><span>Transportation <b>{quotation.transport}</b></span><span>GST ({quotation.gstRate}%) <b>{currency.format(quotation.gstAmount)}</b></span><strong>Quotation total <b>{currency.format(quotation.total)}</b></strong></div></div>
    <div className="secure-quote-actions"><Link className="quotation-primary" href={`/account/quotations/${quotation.id}`}><FileText size={17} /> Download in customer portal</Link><a className="quotation-contact" href={whatsappContactHref(`quotation ${quotation.quoteNumber}`)} target="_blank" rel="noreferrer"><MessageCircle size={17} /> Contact RAC on WhatsApp</a></div>
