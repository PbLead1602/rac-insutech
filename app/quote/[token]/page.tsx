...
export default async function SecureQuotationPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  return <main className="quotation-page"><CatalogueHeader /><section className="secure-quote catalogue-shell">
    ...
    <div className="secure-quote-bottom"><div className="secure-quote-note"><ShieldCheck size={19} /><p><strong>Review required before order acceptance.</strong> Sign in to your approved RAC account to download the protected PDF or request a revision.</p></div><div className="secure-quote-total"><span>Subtotal <b>{currency.format(quotation.subtotal)}</b></span><span>GST ({quotation.gstRate}%) <b>{currency.format(quotation.gstAmount)}</b></span><span>Transport <b>{quotation.transport}</b></span><strong>Total <b>{currency.format(quotation.total)}</b></strong></div></div>
