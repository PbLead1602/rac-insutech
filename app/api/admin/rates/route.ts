...
export async function GET(request: Request) {
  ...
  try { return NextResponse.json({ ok: true, rates: await listAdminRateCards(new URL(request.url).searchParams.get("q") || ""), options: quotationRateOptions() }, { headers: { "Cache-Control": "no-store" } }); } catch (error) { return NextResponse.json({ ok: false, message: error instanceof Error ? error.message : "Could not load rate cards." }, { status: 500 }); }
}
