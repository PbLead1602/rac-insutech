...
export async function POST(request: Request) {
  ...
  try {
    ...
    const analysis = await analyseAdminRateImport({ fileName: file.name, bytes: new Uint8Array(await file.arrayBuffer()), profile, adminId: admin.mode === "live" ? admin.id : undefined });
