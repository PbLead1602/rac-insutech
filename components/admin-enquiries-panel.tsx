...
function EnquiryForm({ onClose, onSubmit }: { onClose: () => void; onSubmit: (draft: Record<string, string>) => Promise<boolean> }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const form = new FormData(event.currentTarget); const draft = Object.fromEntries(["name", "company", "mobile", "email", "city", "district", "state", "pinCode", "projectName", "projectLocation", "product", "brand", "quantity", "thickness", "application", "customerType", "deliveryPreference", "message"].map((field) => [field, String(form.get(field) || "")])); setBusy(true); setError(""); const saved = await onSubmit(draft); if (!saved) setError("Check the customer contact details and try again."); setBusy(false); };
  return <div className="admin-drawer-backdrop" role="presentation" onMouseDown={onClose}><aside className="admin-enquiry-drawer admin-customer-form" role="dialog" aria-modal="true" aria-label="Add enquiry" onMouseDown={(event) => event.stopPropagation()}><header><div><p>ENQUIRY MANAGEMENT</p><h2>Add enquiry</h2><span>Record an offline or phone enquiry and route it into the quotation workflow.</span></div><button type="button" onClick={onClose} aria-label="Close add enquiry"><X size={18} /></button></header><form className="admin-drawer-body admin-customer-fields" onSubmit={submit}><div className="admin-customer-fields-grid"><label>Full name<input name="name" required autoFocus /></label><label>Company<input name="company" /></label><label>Mobile number<input name="mobile" required inputMode="tel" /></label><label>Email<input name="email" type="email" /></label><IndiaLocationFields /><label>Customer type<select name="customerType" defaultValue="end_user"><option value="end_user">End user</option><option value="contractor">Contractor</option><option value="consultant">Consultant</option><option value="dealer">Dealer</option><option value="other">Other</option></select></label><label>Project name<input name="projectName" /></label><label>Project location<input name="projectLocation" /></label><label>Product / material<input name="product" /></label><label>Application<input name="application" /></label><label>Brand (if specified)<input name="brand" /></label><label>Quantity<input name="quantity" /></label><label>Thickness / size<input name="thickness" /></label><label>Delivery preference<input name="deliveryPreference" /></label></div><label>Requirement<textarea name="message" placeholder="Project details, specification, timeline or other useful information." /></label>{error && <p className="admin-form-error">{error}</p>}<div className="admin-customer-form-actions"><button type="button" className="admin-drawer-secondary" onClick={onClose}>Cancel</button><button className="admin-os-primary" disabled={busy}>{busy ? "Saving..." : "Create enquiry"}<ArrowRight size={15} /></button></div></form></aside></div>;
}
