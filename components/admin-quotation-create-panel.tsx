...
/** Presentation-only class for size/packing option cards. */
function batchSizeChecksStyleClass(productId: QuoteProductId) {
  if (isTubeProduct(productId)) return "admin-multiple-pipe-size-checks";
  if (productId === "open-cell-nitrile-rubber-sheet") return "admin-size-packing-checks admin-size-packing-checks--sheet";
  if (productId === "insulation-tape") return "admin-size-packing-checks admin-size-packing-checks--tape";
  if (productId === "insulation-adhesive") return "admin-size-packing-checks admin-size-packing-checks--drum";
  return "admin-size-packing-checks";
}
...
export default function AdminQuotationCreatePanel() {
  ...
  useEffect(() => {
    if (!shouldScrollToBuiltUpBasket.current || !customBuiltUpItems.length) return;
    shouldScrollToBuiltUpBasket.current = false;
    builtUpBasketRef.current?.scrollIntoView({ behavior: "smooth", block: "start" });
  }, [customBuiltUpItems]);
  ...
  const addSelectedConfigurations = () => {
    ...
    setExpandedMobileConfigurationLineId(additions[additions.length - 1].id);
  ...
  return <div className="admin-os-content admin-manual-quotation-content">
    ...
    <form className="admin-customer-fields admin-os-card admin-manual-quotation" onSubmit={submit} key={`${customerMode}-${selectedCustomer?.id || enquiry?.id || "manual-quotation"}`}>
      ...
      <section className="admin-multiple-selection" hidden={isClassONitrileTube(batchSelection.productId) && nitrileMode === "custom"} aria-labelledby="admin-multiple-selection-title">
        ...
        <div className="admin-multiple-selection-grid">
          ...
          <fieldset className="admin-multiple-checks admin-multiple-thickness-checks"><legend>Select thicknesses <em>(one or more)</em></legend><div>{batchThicknesses.map((thickness) => <label key={thickness}><input type="checkbox" checked={batchSelection.thicknesses.includes(thickness)} onChange={() => setBatchSelection((current) => { const thicknesses = toggleSelection(current.thicknesses, thickness); const validSizes = new Set(thicknesses.flatMap((selectedThickness) => quoteOptions(current.productId, "size", { materialClass: current.materialClass, thickness: selectedThickness }))); return { ...current, thicknesses, sizes: current.sizes.filter((size) => validSizes.has(size)), lamination: "" }; })} />{thickness}</label>)}</div></fieldset>
          {requiresBatchSizeSelection(batchSelection.productId) && batchSizeOptions.length > 1 && <fieldset className={`admin-multiple-checks ${batchSizeChecksStyleClass(batchSelection.productId)}`}><legend>Select {isTubeProduct(batchSelection.productId) ? "pipe / roll sizes" : "size / packing"} <em>(one or more)</em></legend><div>{batchSizeOptions.map((size) => <label key={size}><input type="checkbox" checked={batchSelection.sizes.includes(size)} onChange={() => setBatchSelection((current) => ({ ...current, sizes: toggleSelection(current.sizes, size), lamination: "" }))} />{isTubeProduct(batchSelection.productId) ? <span title={size}>{size}</span> : size}</label>)}</div></fieldset>}
      ...
      <section className="admin-selected-configurations" aria-labelledby="admin-selected-configurations-title">
        ...
        {configurationRows.length ? <>
          ...
          <div className="admin-mobile-configuration-cards">{rowCalculations.map((entry, index) => {
            const { row, variant, line, error } = entry;
            const { configuration } = row;
            const materialClasses = quoteOptions(row.productId, "materialClass");
            const thicknesses = quoteOptions(row.productId, "thickness", { materialClass: configuration.materialClass });
            ...
            return <article className={`admin-mobile-configuration-card${isExpanded ? " is-expanded" : ""}`} key={row.id}>
              <div className="admin-mobile-configuration-card-summary">
                <span className="admin-mobile-configuration-line-number">{index + 1}</span>
                <div className="admin-mobile-configuration-product-summary"><strong>{productName}</strong><span>{configuration.thickness}<i />{configuration.lamination}<i />{configuration.materialClass}</span><small>{configuration.size}</small></div>
                <div className="admin-mobile-configuration-subtotal"><strong>{line ? currency.format(line.amount) : "-"}</strong><span>{row.quantity} × {orderUnitLabel}</span><small>{rate !== undefined ? `@ ${currency.format(rate)} per ${variant?.rateUnit || "unit"}` : error || "Rate pending"}</small></div>
                <button type="button" className="admin-mobile-configuration-remove" onClick={() => removeRow(row.id)} aria-label="Remove quotation line" title="Remove line"><Trash2 size={16} /></button>
                <button type="button" className="admin-mobile-configuration-toggle" onClick={() => setExpandedMobileConfigurationLineId((current) => current === row.id ? null : row.id)} aria-expanded={isExpanded} aria-label={isExpanded ? "Collapse configuration line" : "Expand configuration line"}>{isExpanded ? <ChevronUp size={20} /> : <ChevronDown size={20} />}</button>
              </div>
              {isExpanded && <div className="admin-mobile-configuration-fields">
