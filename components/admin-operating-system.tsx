...
type AdminQuotation = { id: string; quoteNumber: string; company: string; total: number; createdAt: string; status: string; validUntil?: string; followUpAt?: string };
...
type DashboardData = { products: number; enquiries: number; quotations: number; pendingApprovals: number; quotedValue: number; newQuotationsToday: number; followUpsDue: number; quotesExpiringSoon: number; wonValue: number; recent: Enquiry[]; recentQuotations: AdminQuotation[]; attention: Attention[] };
...
const emptyDashboard: DashboardData = { products: 0, enquiries: 0, quotations: 0, pendingApprovals: 0, quotedValue: 0, newQuotationsToday: 0, followUpsDue: 0, quotesExpiringSoon: 0, wonValue: 0, recent: [], recentQuotations: [], attention: [] };
...
const isOpenQuotation = (status: string) => !["won", "lost", "expired", "cancelled"].includes(status);
const quotationDashboardMetrics = (quotations: Array<Pick<AdminQuotation, "createdAt" | "followUpAt" | "status" | "total" | "validUntil">>) => {
  const today = indiaDateKey(new Date());
  const expiringBy = indiaDateAfterDays(today, 7);
  return {
    newQuotationsToday: quotations.filter((item) => indiaDateKey(item.createdAt) === today).length,
    followUpsDue: quotations.filter((item) => isOpenQuotation(item.status) && item.followUpAt && indiaDateKey(item.followUpAt) <= today).length,
    quotesExpiringSoon: quotations.filter((item) => {
      const validUntil = item.validUntil?.slice(0, 10);
      return isOpenQuotation(item.status) && validUntil && validUntil >= today && validUntil <= expiringBy;
    }).length,
    wonValue: quotations.filter((item) => item.status === "won").reduce((total, item) => total + Number(item.total || 0), 0),
  };
};
...
function Dashboard({ dashboard, go }: { dashboard: DashboardData; go: (href: string) => void }) {
  const kpis: Array<{ label: string; value: string | number; hint: string; icon: LucideIcon; href: string }> = [{ label: "New enquiries today", value: dashboard.attention[0]?.value || 0, hint: `${dashboard.enquiries} total enquiries`, icon: ClipboardList, href: "/admin/enquiries" }, { label: "New quotations today", value: dashboard.newQuotationsToday, hint: `${dashboard.quotations} total quotations`, icon: FileText, href: "/admin/quotations" }, { label: "Open quotations", value: dashboard.attention[1]?.value || 0, hint: "Awaiting review or follow-up", icon: HandCoins, href: "/admin/quotations" }, { label: "Follow-ups due", value: dashboard.followUpsDue, hint: "Today and overdue follow-ups", icon: BellRing, href: "/admin/quotations" }, { label: "Quotes expiring soon", value: dashboard.quotesExpiringSoon, hint: "Valid through the next 7 days", icon: Activity, href: "/admin/quotations" }, { label: "Quoted value", value: currency(dashboard.quotedValue), hint: "All generated quotations", icon: BriefcaseBusiness, href: "/admin/quotations" }, { label: "Won value", value: currency(dashboard.wonValue), hint: "Quotations marked won", icon: ShieldCheck, href: "/admin/quotations" }, { label: "Active products", value: dashboard.products, hint: "Public catalogue records", icon: Boxes, href: "/admin/products" }];
