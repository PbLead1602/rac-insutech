...
async function loadDashboard(): Promise<DashboardData> {
  ...
  if (!client) {
    ...
    const quotations = quoteData.quotations || []; const enquiries = enquiryData.enquiries || []; const products = productData.products || []; const today = new Date().toDateString();
    const newEnquiries = enquiries.filter((item) => new Date(item.createdAt).toDateString() === today).length;
  ...
  const [productResult, enquiryResult, accountResult, quoteResult, recentEnquiries, recentQuotes] = await Promise.all([client.from("products").select("id", { count: "exact", head: true }), client.from("enquiries").select("id", { count: "exact", head: true }), client.from("customer_accounts").select("id", { count: "exact", head: true }).eq("approval_status", "pending_admin_approval"), client.from("quotations").select("id, total, status"), client.from("enquiries").select("name, product_name, created_at, status").order("created_at", { ascending: false }).limit(5), client.from("quotations").select("id, quote_number, customer, total, created_at, status").order("created_at", { ascending: false }).limit(6)]);
